...
Because the sail boat interacts between two fluids not only do they generate forces but also resistances in both mediums. It is important to know how the elements are named and which words are related with specific axis and  %These simple concepts are useful to understand how the equilibrium is generated in the static condition, 
...
\begin{figure} %[ht]
  \centering
  \subfloat[Motion depict the by mathematical model.]{\includegraphics[width=0.38\linewidth]{ridder_motion.png}\label{fig:motion_csys_tackmodel}}
  \hfill
  \subfloat[Angle's location and axis orientation according the coordinate system.]{\includegraphics[width=0.45\hsize]{ridder_caxis.png}\label{fig:axis_cystackmodel}}
...
From table \ref{table:SNAME_notation} it was define that $\psi$ is related with \textit(r) the yaw angular velocity, and it defines the position or orientation of the boat, which means that it set the direction of the course ($\lambda$).  $\lambda$ is considered a behaviour variables(\textit{$y_{b}$}) on section \ref{sec:VPP}. % it was mentioned 
